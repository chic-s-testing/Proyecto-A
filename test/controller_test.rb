...
class GameControllerTest < Test::Unit::TestCase
  ...
  def test_select_coordinates_remove_flag
    status = @controller.select_coordinates(0, 0, 'delete flag')
    assert_false(@model.board[0][0].flag)
    assert_equal('next', status)
  end

  def test_select_coordinates_uncover_is_loser
    status = @controller.select_coordinates(0, 0, 'uncover cell')
  ...
  def test_select_coordinates_uncover_is_winner
    (0...10).each do |row|
      (0...10).each do |col|
        @controller.select_coordinates(row, col, 'uncover cell') unless [row, col] == [0, 0] || [row, col] == [9, 9]
      end
    end
    @view.print_board(@model.board)
    status = @controller.select_coordinates(9, 9, 'uncover cell')
